...
- [ ] **The editor logs `Node not found` once per input on group, ungroup, CSG and paste.** Cosmetic
	  — the operations all work, and the message names brushes the action legitimately removed — but
	  it is noise in every user's Output panel and it should not be shipped indefinitely.
  - **Bisected to `a7c3125` (Derive collision, occlusion and UV2).** `dff2ff0` is clean, `a7c3125`
	is not, so it has fired on every group since the derived-node model landed. It went unnoticed
	because nobody watches the Output during a group.
  - **The trigger needs BOTH halves, and each alone is harmless.** Isolated by stubbing out parts of
	`group_ops.group()` and testing each variant in the editor — clean unless marked:
	- remove the brushes, build no group — clean
	- build the group *and* remove the brushes, but skip `absorb_world` — clean
	- the full operation — **one error per removed brush, repeating**
	- delete brushes (their own action), then add any node later — clean
	- delete brushes, then draw a brush (which raises an unowned subtree) — clean

	So it takes a removal *inside an undo action that also adds*, followed by an `add_child` of the
	**unowned** subtree `Collision.ensure_tree` raises. Neither ingredient offends on its own, which
	is why every reading of the code missed it.
  - **Not a timing problem, though it looks like one.** Deferring the subtree raise to after
	`commit_action`, then by one frame, then by three, all still logged. Drawing a brush by hand
	seconds later is clean, so the dock does recover — but no delay we tried lands outside the
	window. Do not re-try "wait a bit longer" without new evidence.
  - **Disproven, so nobody re-runs them:** the inspector's `MultiNodeEdit` holding node paths
	(cleared, plus `inspect_object(null)`, a full frame ahead of the commit — no change); the
	selection being stale (same); the removal itself (clean in isolation); ordering the adds before
	the removes (no change); `godot_mcp` (its `_get_node` calls are all request-handlers, it watches
	neither tree nor selection). Duckboard itself resolves no paths at all — no `get_node`, no
	`NodePath` anywhere in the addon — so the lookup is the editor's own.
  - **The decision this actually asks for** is not a fix to `group()`. It is whether the derived
	subtree can be raised somewhere the Scene dock does not object to, or whether one warning per
	grouped solid is an acceptable cost of the model `a7c3125` chose deliberately. That belongs in
	`collision.gd`, next to `ensure_tree`, since that is where every unowned child in the addon is
	born.
- [ ] **Ungroup logs `Condition "!is_inside_tree()" is true` from `node_3d.cpp`, two or three times.**
	  Almost certainly the same family as the entry above — it appeared and disappeared alongside it
	  through the whole investigation — but it was never isolated on its own, so treat that as a
	  guess rather than a finding.
  - Every obvious candidate is already guarded and was checked: `Brush._notification` and
	`BrushGroup._notification` both test `is_inside_tree()`, `BrushGroup._to_world()` returns
	identity when detached, and both `recenter()` implementations return early. Configuring a brush
	off-tree, `set_world_faces`, `absorb_world`, `rebuild_mesh` and `kernel_for` on a detached group
	were all exercised headlessly and none of them logs it.
  - Two places DO read `global_transform` without a guard and are worth ruling in or out first:
	`Duckboard._brush_world_aabb` (called from the overlay draw, which runs while a removed node can
	still be selected) and `GroupIsolate.sync` (per redraw, guarded on `is_instance_valid` only).
	Guarding both was tried and did not silence it, so they are suspects, not the cause.
